<?php
// updatechar.php
include('config.php');
function bersihkan_input($conn, $data) {
    $data = trim($data);
    $data = htmlspecialchars($data, ENT_QUOTES, 'UTF-8');
    $data = $conn->real_escape_string($data);
    return $data;
}

// Menangkap ID Primary Key
$char_id    = (int)bersihkan_input($conn, $_POST['char_id']);

$uid        = bersihkan_input($conn, $_POST['uid']);
$name       = bersihkan_input($conn, $_POST['name']);
$server     = bersihkan_input($conn, $_POST['server']);
$game       = bersihkan_input($conn, $_POST['game']);
$guild      = bersihkan_input($conn, $_POST['guild']);
$class      = bersihkan_input($conn, $_POST['class']);
$social     = bersihkan_input($conn, $_POST['social']);
$quote      = bersihkan_input($conn, $_POST['quote']);
$avatar     = bersihkan_input($conn, $_POST['avatar']);
$preview    = bersihkan_input($conn, $_POST['preview']);
$input_pass = bersihkan_input($conn, $_POST['password']); 

// 1. Ambil password asli berdasarkan ID Primary Key
$check_stmt = $conn->prepare("SELECT password FROM characters WHERE id = ?");
$check_stmt->bind_param("i", $char_id);
$check_stmt->execute();
$result = $check_stmt->get_result();

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $db_password = $row['password'];

    // 2. Cocokkan password teks biasa
    if ($input_pass === $db_password) {
        
        // Eksekusi update data berdasarkan ID Primary Key
        $stmt = $conn->prepare("UPDATE characters SET uid=?, name=?, server=?, game=?, guild=?, class=?, social=?, password=?, quote=?, avatar=?, preview=? WHERE id=?");
        $stmt->bind_param("sssssssssssi", $uid, $name, $server, $game, $guild, $class, $social, $input_pass, $quote, $avatar, $preview, $char_id);
        
        if ($stmt->execute()) {
            http_response_code(200);
        } else {
            http_response_code(500);
        }
        $stmt->close();

    } else {
        http_response_code(403); // Password Salah
    }
} else {
    http_response_code(404);
}

$check_stmt->close();
$conn->close();
?>